...
export default function EmergencyInfoPage() {
  const [tab, setTab] = useState<"insurance" | "contacts" | "hospitals" | "vets">("insurance");
  ...
  // Modal state (shared for contacts + facilities)
  const [modalType, setModalType] = useState<"contact" | "hospital" | "vet" | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [iRes, cRes, fRes] = await Promise.all([
  ...
  function openAddFacility(type: "HOSPITAL" | "VET") {
    setEditingId(null);
    setFacilityForm(EMPTY_FACILITY(type));
    setModalError("");
    setModalType(type === "HOSPITAL" ? "hospital" : "vet");
  }
  ...
  function openEditFacility(f: EmergencyFacility) {
    ...
    setModalError("");
    setModalType(f.type === "HOSPITAL" ? "hospital" : "vet");
  }
  ...
  async function saveFacility() {
    ...
    try {
      const payload = {
        ...
        distance: facilityForm.distance.trim() || null,
  ...
  async function deleteFacility(id: string) {
    ...
    setDeletingId(id);
    try {
      await fetch(`/api/emergency/facilities/${id}`, { method: "DELETE" });
  ...
  const isContactModal = modalType === "contact";
  const isFacilityModal = modalType === "hospital" || modalType === "vet";
        ...
        <>
              ...
              <form onSubmit={saveInsurance} className="card p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2">
                    ...
                    <input type="text" value={insurance.website ?? ""} onChange={(e) => setInsurance((p) => ({ ...p, website: e.target.value }))} className="input w-full" placeholder="e.g. www.bcbs.com or https://..." />
                    <p className="text-xs text-slate-400 mt-0.5">Any URL format works — https:// will be added automatically if missing.</p>
                  </div>
                  <div className="col-span-2">
          ...
          {tab === "contacts" && (
            <div className="max-w-2xl space-y-4">
              ...
              {contacts.length === 0 ? (
                <EmptyState emoji="📞" message="No emergency contacts yet" sub="Add family members, doctors, or other key contacts." onAdd={openAddContact} addLabel="Add Contact" />
              ) : (
          ...
          {tab === "hospitals" && (
            <FacilityTab
              ...
              onAdd={() => openAddFacility("HOSPITAL")}
              onEdit={openEditFacility}
          ...
          <div className="space-y-4 px-6 py-5">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" value={contactForm.phone} onChange={(e) => setContactForm((f) => ({ ...f, phone: e.target.value }))} className="input w-full" placeholder="e.g. (555) 123-4567" />
              </div>
              <div className="col-span-2">
                <label className="block text-xs font-semibold text-slate-600 mb-1">Email</label>
                <input type="text" value={contactForm.email} onChange={(e) => setContactForm((f) => ({ ...f, email: e.target.value }))} className="input w-full" placeholder="e.g. doctor@clinic.com" />
              </div>
              <div className="col-span-2">
                <label className="block text-xs font-semibold text-slate-600 mb-1">Notes</label>
...
function FacilityTab({ title, emoji, type, facilities, onAdd, onEdit, onDelete, deletingId }: {
  ...
  onAdd: () => void;
  onEdit: (f: EmergencyFacility) => void;
            ...
            <div key={f.id} className="px-5 py-4 flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {f.isPreferred && (
                    <span className="text-xs bg-brand-100 text-brand-700 font-semibold px-2 py-0.5 rounded-full">⭐ Preferred</span>
                  )}
                </div>
